/*
 * NullSec RKHunt v2.5 - Advanced Rootkit Hunter
 * Comprehensive rootkit detection with extensive signature database
 * 
 * Compile: gcc -O2 -Wall -D_GNU_SOURCE -o rkhunt rkhunt.c -lpthread -lcrypto
 * License: MIT
 */
...
C_CYAN "  ╭──────────────────────────────────────────╮\n"
"  │" C_WHITE "  RKHunt " C_DIM "v2.5" C_CYAN "  │  " C_GRAY "Advanced Rootkit Hunter" C_CYAN "  │\n"
"  │" C_DIM "     github.com/bad-antics/nullsec-rkhunt" C_CYAN " │\n"
"  ╰──────────────────────────────────────────╯" C_RESET "\n";
...
int main(int argc, char *argv[]) {
    ...
    for (int i = 1; i < argc; i++) {
        if (strcmp(argv[i], "-h") == 0 || strcmp(argv[i], "--help") == 0) {
            ...
            printf("RKHunt v%s - Advanced Rootkit Hunter\n", VERSION);
    ...
    if (log_path) {
        ...
        if (!log_file) {
            ...
            time_t now = time(NULL);
            fprintf(log_file, "RKHunt v%s - Scan started at %s\n", VERSION, ctime(&now));
        }
    }
